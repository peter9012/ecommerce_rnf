...
public class AddShippingTest extends RFWebsiteBaseTest{
	...
	//Hybris Phase 2: Pop up to update autoship shipping profile on changing default selection
	@Test
	...
	//ADD/EDIT a shipping profile from AD-HOC CHECKOUT page. (There is NO checkbox.)
	@Test(enabled=false)
	public void testEditShippingProfileNoCheckbox() throws InterruptedException{
		int randomNum = CommonUtils.getRandomNum(10000, 1000000);
		String consultantEmailID = null;
		...
		storeFrontConsultantPage = storeFrontHomePage.loginAsConsultant(consultantEmailID, TestConstants.CONSULTANT_PASSWORD_TST4);
		s_assert.assertTrue(storeFrontConsultantPage.verifyConsultantPage(),"PC User Page doesn't contain Welcome User Message");
		logger.info("login is successful");
	...
	//Edit the billing profile and add a new billing address
	@Test(enabled=false)
	public void testAddNewBillingAddress() throws InterruptedException
	{
		int randomNum = CommonUtils.getRandomNum(10000, 1000000);
		RFL_DB = driver.getDBNameRFL();
		RFO_DB = driver.getDBNameRFO();
		...
		String newShippingAddressName = TestConstants.NEW_ADDRESS_NAME_US+randomNum;
		String lastName = "ln";
		storeFrontUpdateCartPage.enterNewBillingAddressName(newShippingAddressName+" "+lastName);
		storeFrontUpdateCartPage.enterNewBillingAddressLine1(TestConstants.NEW_ADDRESS_LINE1_US);
		...
		storeFrontUpdateCartPage.enterNewBillingAddressPostalCode(TestConstants.CONSULTANT_POSTAL_CODE);
		storeFrontUpdateCartPage.enterNewBillingAddressPhoneNumber(TestConstants.NEW_ADDRESS_PHONE_NUMBER_US);
		storeFrontUpdateCartPage.clickOnSaveBillingProfile();
		//s_assert.assertTrue(driver.findElement(By.xpath("//input[@id='billingAddressattention']")).getText().equalsIgnoreCase(driver.findElement(By.xpath("//ul[@id='multiple-billing-profiles']/li[1]/p[1]/span[1]")).getText()), "Addresses should Match");
