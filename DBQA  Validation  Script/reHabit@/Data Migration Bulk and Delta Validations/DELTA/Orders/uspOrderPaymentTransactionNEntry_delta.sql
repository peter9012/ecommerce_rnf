...
USE DM_QA
GO
CREATE PROCEDURE dbqa.uspOrderPaymentTransactionNEntry_delta
    (
      @StartDate DATE ,
...
AS
    BEGIN
        ...
        WHERE   EXISTS ( SELECT 1
                ...
                AND opt.ServerModifiedDate BETWEEN @StartDate
                                           AND     @EndDate
        ...
        FROM    Hybris.dbo.PaymentTransactions p
                ...
                JOIN Hybris.dbo.composedtypes ct ON ct.pk = ho.TypePkString
                                                    AND ct.InternalCode = 'Order'
        WHERE   ( ( p.ModifiedTS BETWEEN @StartDate AND @EndDate )
                  OR ( pte.modifiedTS BETWEEN @StartDate AND @EndDate )
                )
...
--++++++++++++++++++--++++++++++++++++++
        SET @message = CONCAT(' STEP: 5.initiating DUPLICATE Need to updaete Scripts Need to delete from Temps for End to end too.  ',
                              CHAR(10),
                              '-----------------------------------------------')
        EXECUTE dbqa.uspPrintMessage @message
