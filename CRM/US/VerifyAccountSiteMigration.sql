...
IF OBJECT_ID('CRM.SFDC.SiteDifference') IS NOT NULL  DROP TABLE CRM.SFDC.SiteDifference
IF OBJECT_ID('CRM.SFDC.SiteMissing') IS NOT NULL DROP TABLE CRM.SFDC.SiteMissing
IF OBJECT_ID('CRM.SFDC.RFO_Site') IS NOT NULL DROP TABLE CRM.SFDC.RFO_Site
IF OBJECT_ID('CRM.SFDC.CRM_Site') IS NOT NULL DROP TABLE CRM.SFDC.CRM_Site
IF OBJECT_ID('CRM.SFDC.ErrorLog_Site') IS NOT NULL DROP TABLE CRM.SFDC.ErrorLog_Site
...
SELECT MissingFrom ,COUNT(*) from CRM.SFDC.SiteMissing GROUP BY MISSINGFROM;

SELECT 'Query CRM.SFDC.SiteMissing to get list of AccountIDs missing from Source/Destination'
...
CAST(DATEADD(HH,(SELECT OFFSET FROM  CRM.SFDC.GMT_DST M WHERE StartDate >= M.DST_START AND StartDate < M.DST_END),StartDate) AS DATE) as StartDate__c,
CAST(DATEADD(HH,(SELECT OFFSET FROM  CRM.SFDC.GMT_DST M WHERE EndDate >= M.DST_START AND EndDate< M.DST_END),EndDate) AS DATE) as EndDate__c,
CAST(DATEADD(HH,(SELECT OFFSET FROM  CRM.SFDC.GMT_DST M WHERE s.ServerModifiedDate >= M.DST_START AND s.ServerModifiedDate< M.DST_END),s.ServerModifiedDate) AS DATE) as ServerModifiedDate__c,
...
CAST(DATEADD(HH,(SELECT OFFSET FROM  CRM.SFDC.GMT_DST M WHERE Expirationdate >= M.DST_START AND Expirationdate< M.DST_END),Expirationdate) AS DATE) as Expirationdate__c,
PrimaryDomain__C,
BusinessDomain__c,
MailAddress as EMailAddress__c
INTO CRM.SFDC.RFO_Site
...
(SELECT SU.SITEID,MAX(CASE WHEN SU.SITEDOMAINID=1 THEN SD.NAME ELSE NULL END) AS PRIMARYDOMAIN__C,MAX(CASE WHEN SU.SITEDOMAINID=2 THEN SD.NAME ELSE NULL END) AS BusinessDOMAIN__C
 ...
 WHERE NOT EXISTS (SELECT 1 FROM CRM.SFDC.SiteMissing SD WHERE SD.RFO_AccountId=S.ACCOUNTID and missingfrom='Destination')
...
SELECT * INTO #Site FROM CRM.SFDC.RFO_Site
EXCEPT 
SELECT * FROM CRM.SFDC.CRM_Site

--SELECT * FROM CRM.SFDC.CRM_Site
--SELECT * FROM CRM.SFDC.RFO_Site

CREATE TABLE CRM.SFDC.ErrorLog_Site
(
...
DECLARE @I INT = (SELECT MIN(ColID) FROM  CRM.SFDC.CRM_METADATA WHERE CRMObject = 'Site') , 
@C INT =  (SELECT MAX(ColID) FROM  CRM.SFDC.CRM_METADATA WHERE CRMObject = 'Site') 
...
DECLARE @DesTemp NVARCHAR (50) ='CRM.SFDC.CRM_Site' 
...
' FROM CRM.SFDC.ErrorLog_Site a  JOIN ' +@DesTemp+
...
' INSERT INTO CRM.SFDC.ErrorLog_Site (Identifier,ColID,RecordID,RFO_Value) ' + @SQL1  + @SQL2
...
FROM CRM.SFDC.ErrorLog_Site A JOIN CRM.SFDC.CRM_METADATA B ON a.ColID =b.ColID
