...
--Orderitem counts HYbris & RFO
        ...
        FROM    RFOSERVER.RFOperations.Hybris.Orders o          
                INNER JOIN RFOSERVER.RFOperations.Hybris.OrderItem oi ON oi.OrderId = o.OrderID
                INNER JOIN hybris..products p ON p.pk = oi.ProductID
                INNER JOIN #LoadedOrders ho ON ho.pk = o.OrderID
               

       

        SELECT  COUNT(DISTINCT oe.PK)
        FROM    Hybris.dbo.orders o ( NOLOCK )
                INNER JOIN Hybris..orderentries oe ON oe.orderpk = o.pk
				JOIN #LoadedOrders ho ON ho.pk=o.PK
...
-- Missing 
 -----------------------------------------------------------------------------------------------------------------------------
        ...
        IF OBJECT_ID('Hybris.dbo.OrderItemsMissing') IS NOT NULL
            DROP TABLE Hybris.dbo.OrderItemsMissing;
        ...
        SELECT  OrderItemID AS RFO_OrderItemID ,
                ...
                END AS MissingFROM
        INTO    Hybris.dbo.OrderItemsMissing
        ...
        FROM    Hybris.dbo.OrderItemsMissing;
...
----------------------------------------------------------------------------------------------------------------------
        ...
        FROM    Hybris.dbo.orders o ( NOLOCK )
                INNER JOIN Hybris..orderentries oe ON oe.orderpk = o.pk
				INNER JOIN #loadedorders ho ON ho.pk=o.PK
...
-------------------------------------------------------------------------------
       ...
       INTO    #RFO_Item
        FROM    rfoserver.RFOperations.Hybris.OrderItem a
                JOIN rfoserver.RFOperations.Hybris.ProductBase b ON a.ProductID = b.ProductID
...
SET a.Hybris_Value = b. ' + @DesCol
        ...
        FROM    Hybris.dbo.ErrorLog_Orders a
                JOIN Hybris.dbo.Metadata_orders b ON a.MapID = b.MapID
        GROUP BY a.MapID ,
                RFO_column
	...
	SELECT * FROM hybris.dbo.orderentries oe
	JOIN RFOSERVER.RFOperations.hybris.orderItem it ON oe.pk=it.OrderItemID
	JOIN #LoadedOrderItems lo ON lo.pk=oe.PK
	WHERE oe.info<> 