...
SELECT ' Refer Following tables to view test results.
		1. CRM.SFDC.PaymentProfileDifference--> Difference in RFO and CRM Accounts
...
SELECT
		...
		CCV.NAME AS Vendor,
		CAST(CASE WHEN DATEADD(HH,(SELECT OFFSET FROM  CRM.SFDC.GMT_DST M WHERE PP.StartDate >= M.DST_START AND PP.StartDate < M.DST_END),PP.StartDate) IS NULL THEN '1900-01-01' ELSE pp.StartDate END AS DATE) StartDate,
		CAST(CASE WHEN DATEADD(HH,(SELECT OFFSET FROM  CRM.SFDC.GMT_DST M WHERE PP.EndDate >= M.DST_START AND PP.EndDate < M.DST_END),PP.EndDate) IS NULL THEN '1900-01-01' ELSE pp.EndDate END AS DATE) EndDate,
