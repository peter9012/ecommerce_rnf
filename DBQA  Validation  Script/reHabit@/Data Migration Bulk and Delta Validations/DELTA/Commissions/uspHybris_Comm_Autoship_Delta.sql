...
CREATE PROCEDURE [dbqa].[uspHybris_Comm_Autoship_Delta] (@StartDate DATE,@EndDate DATETIME=NULL)
AS
    BEGIN
...
SET @EndDate=ISNULL(@EndDate,GETDATE()-1)
        ...
        DECLARE @SourceCount INT ,
            ...
            @Flows NVARCHAR(50)= 'Hybris_Comn_Delta' ,
            @owner NVARCHAR(50)= 'Comm_Autoships' ,
            @Key NVARCHAR(25)= 'AutoshipID' ,
       ...
       SELECT  c.p_code [SourceKey] ,
        v.Code [p_type] ,
        u.p_customerid AS p_userpk ,
        c.createdTS ,
        s.Code [p_status] ,
        c.modifiedTS
INTO    #Source
FROM    Hybris.dbo.carts c
        ...
        LEFT  JOIN Hybris.dbo.enumerationvalues v ON v.pk = c.p_carttype
        LEFT JOIN Hybris.dbo.users u ON u.pk = c.p_user
        LEFT JOIN Hybris.dbo.enumerationvalues s ON s.pk = c.p_status
WHERE   c.modifiedTS BETWEEN @StartDate AND @EndDate
        AND v.Code IN ( 'AUTOSHIP_CART' )
...
SELECT CAST([AutoshipID] AS NVARCHAR(225)) [TargetKey]
      ,[AutoshipTypeID]
      ,[AccountID]
      ,[CreationDate]
      ,[AutoshipStatusID]
      ...
      ,[CreatedOn]
      ,[UpdatedOn]
  FROM [Commissions].[etl].[Autoships]
