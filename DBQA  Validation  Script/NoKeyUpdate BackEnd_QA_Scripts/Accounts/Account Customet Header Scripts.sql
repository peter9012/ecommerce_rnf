...
FROM    RFOperations.RFO_Accounts.AccountBase ab
        LEFT JOIN ( SELECT a.AccountID FROM  RFOperations.[Hybris].[Autoship] A 
		 JOIN RodanFieldsLive.dbo.AutoshipOrders ao  ON ao.TemplateOrderID = a.AutoshipID
...
SELECT TOP 1 * FROM Hybris.dbo.Users (NOLOCK) u WHERE p_country =8796100624418  AND p_sourcename = 'Hybris-DM'
