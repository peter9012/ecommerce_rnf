...
CREATE PROCEDURE Migration.Migration_Hybris_OrderBillingAddess_QA  @LastRun DATETIME = '2014-05-01'	
AS
    BEGIN
...
-----------------------------------------------------
        ...
        FROM    RFOperations.Hybris.Orders (NOLOCK) a
                ...
                INNER JOIN Hybris..orders ho ON ho.code = a.OrderNumber
...
----------------------------------------------------------
        ...
        WHERE   EXISTS ( SELECT 1
                         ...
                         WHERE  lba.p_rfaddressid = a.OrderBillingAddressID )
...
-------------------------------------------------------------------
        ...
        WHERE   EXISTS ( SELECT 1
                         FROM   #LoadedBillingAddress lba
                         WHERE  lba.p_rfaddressid = a.p_rfaddressid )


        CREATE CLUSTERED INDEX MIX_HYBladr ON #Hybris_Bladr (P_rfaddressid)
