...
SELECT COUNT(*) AS TemplatesLoaded FROM #LoadedAutoshipID					  
        ...
        ( SELECT    COUNT(DISTINCT asp.AutoshipPaymentID) rfo_cnt
          FROM      RFOperations.Hybris.Autoship a
                    ...
                    JOIN Hybris.dbo.users u ON u.p_rfaccountid = CAST(a.AccountID AS NVARCHAR)
                                               AND u.p_sourcename = 'Hybris-DM'
                    JOIN RFOperations.Hybris.AutoshipPayment asp ON asp.AutoshipID = a.AutoshipID AND LEN(asp.AutoshipPaymentID)>12
					JOIN hybris..orders ho ON ho.pk=a.AutoshipID AND ho.p_template=1
                    JOIN RodanFieldsLive.dbo.OrderPayments lop ON lop.OrderID = ho.code
...
FROM    ( SELECT    COUNT(hpi.PK) hybris_cnt ,
        ...
        JOIN ( SELECT   COUNT(asp.AutoshipPaymentID) rfo_cnt ,
               ...
               WHERE    a.AutoshipID NOT IN ( SELECT   AutoshipID FROM #missing  )--Excluding Missing in Hybris.
               GROUP BY a.AutoshipID
             ) t2 ---986577
 ...
 */
        ...
        SELECT  *
        FROM    ( SELECT    COUNT(ad.PK) Hybris ,
                            ho.PK
                  FROM      Hybris..paymentinfos pa
                            JOIN Hybris..orders ho ON ho.PK = pa.OwnerPkString
                            JOIN Hybris..addresses ad ON ad.PK = pa.p_billingaddress
                                                         AND ho.p_template = 1
                                                         AND pa.duplicate = 1 --AND ho.currencypk=8796125855777 
                                                         AND pa.p_sourcename = 'hybris-DM' --AND ho.pk=8840469970989
                  GROUP BY  ho.PK
                ) t
                JOIN ( SELECT   COUNT(apa.AutoshipPaymentAddressID) Rfo ,
                                apa.AutoShipID
                       FROM     Hybris..orders ho
                                JOIN RFOperations.Hybris.AutoshipPaymentAddress apa ON apa.AutoShipID = ho.PK
                                                              AND ho.p_template = 1
                                JOIN RFOperations.Hybris.AutoshipPayment p ON p.AutoshipID = ho.PK
                                                              AND LEN(AutoshipPaymentID) > 12
                                                              AND LEN(apa.AutoshipPaymentAddressID) > 12
                       GROUP BY apa.AutoShipID
                     ) t1 ON t1.AutoShipID = t.PK
        WHERE   t.Hybris <> t1.Rfo AND t.Hybris<2
        ...
        WHERE   t.Hybris <> t1.Rfo AND t.Hybris>1
...
INSERT  INTO DataMigration.dbo.ExecResult
        SELECT  'Autoship shippingAddress WithKey' AS Entity ,
		'Counts,Dups,Missing' AS Types,
                @StartedTime AS StartedTime ,
                @EndTime AS CompletionTime ,
                DATEDIFF(MINUTE, @StartedTime, @EndTime) AS [Total Time (MM)] ,
...
INSERT  INTO DataMigration.dbo.ExecResult
        SELECT  'All Entities Related to Autoship WithKey' AS Entity ,
                'Counts,Dups,Missing' AS Types ,
                @StartedTime AS StartedTime ,
                @EndTime AS CompletionTime ,
                 DATEDIFF(MINUTE, @StartedTime, @EndTime) AS [Total Time (MM)] ,
