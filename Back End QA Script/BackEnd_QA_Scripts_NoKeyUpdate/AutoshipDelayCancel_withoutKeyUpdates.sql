...
--===============================================================================
        ...
        SELECT  'Column2ColumnValidation' AS TypeOfTesting ,
		...
		SELECT DISTINCT ''' + [owner] + ''', ''' + flag + ''', '''
                            + RFO_Column + ''', ''' + Hybris_Column + ''', A.'
                            + @HYB_key + ', A.' + Hybris_Column + ', B.'
...
---==============================================================================================================
        ...
        SELECT  'ManualFlagFieldValidation' AS TypeOfTesting ,
                GETDATE() AS Startedtime;
...
WHERE   b.PK IS NULL
        ...
        SELECT  'ManualRefFlagFieldValidation' AS TypeOfTesting ,
                GETDATE() AS Startedtime;
        ...
        WHERE   accountId IN (
				...
				CREATE CLUSTERED INDEX cls2 ON #Accounts(accountId)
                  
        BEGIN	
            SET @HYB_key = 'P_rfaccountId';
            SET @RFO_key = 'AccountId';
            ...
            WHILE @cnt <= @lt_1
                BEGIN
                    ...
                    SELECT  @sql_gen_1 = 'SELECT DISTINCT  ''' + [owner]
                            ...
                            + ''' as hybris_column, A.' + @HYB_key
                            + ' as hyb_key, A.' + Hybris_Column
                            + ' as hyb_value, B.' + CAST(@RFO_key AS NVARCHAR)
                            + ' as rfo_key, B.' + RFO_Column + ' as rfo_value

	FROM (SELECT ' + @HYB_key + ', ' + Hybris_Column + ' FROM hybris.dbo.'
                            + Hybris_Table + ' a, #Accounts b where a.'
                            + @HYB_key + '=b.' + CAST(@RFO_key AS NVARCHAR)
                            + '
	except
	SELECT a.' + CAST(@RFO_key AS NVARCHAR) + ', ' + RFO_Column
                            + ' FROM rfoperations.' + [Schema] + '.'
                            + RFO_Table + ' a, #Accounts b where a.'
                            + CAST(@RFO_key AS NVARCHAR) + '=b.' + @RFO_key
                            + ') A  
	...
	(SELECT a.' + CAST(@RFO_key AS NVARCHAR) + ', ' + RFO_Column
                            + ' FROM rfoperations.' + [Schema] + '.'
                            + RFO_Table + ' a, #Accounts b where a.'
                            + CAST(@RFO_key AS NVARCHAR) + '=b.'
                            + CAST(@RFO_key AS NVARCHAR) + '
	except
	SELECT ' + @HYB_key + ', ' + Hybris_Column + ' FROM hybris.dbo.'
                            + Hybris_Table + ' a, #Accounts b where a.'
                            + @HYB_key + '=b.' + @RFO_key + ') B
	ON A.' + @HYB_key + '=B.' + CAST(@RFO_key AS NVARCHAR) + '
	UNION
	SELECT DISTINCT ''' + [owner] + ''', ''' + flag + ''', ''' + RFO_Column
                            + ''', ''' + Hybris_Column + ''', A.' + @HYB_key
                            + ', A.' + Hybris_Column + ', B.'
                            + CAST(@RFO_key AS NVARCHAR) + ',B.' + RFO_Column
                            + '
	...
	SELECT a.' + @RFO_key + ', ' + RFO_Column + ' FROM rfoperations.'
                            + [Schema] + '.' + RFO_Table
                            + ' a, #Accounts b where a.'
                            + CAST(@RFO_key AS NVARCHAR) + '=b.'
                            + CAST(@RFO_key AS NVARCHAR) + ') A  
...
--				============================================
        ...
        SELECT  'ManuallySourceNameValidation' AS TypeOfTesting ,
                GETDATE() AS Startedtime;
        BEGIN
        ...
        END;
        SELECT  '985-AutoShipDelayCancel' AS AreaToValidate ,
                ( SELECT    COUNT(*)
                ...
                ) AS TotalfieldsFailled ,
                ( SELECT    COUNT(*)
                  FROM      DataMigration..map_tab
                  WHERE     [owner] = '985-ASDC'
                            AND ( prev_run_err = 0
                                  OR prev_run_err IS  NULL
                                )
                ) AS TotalFieldPassed;
