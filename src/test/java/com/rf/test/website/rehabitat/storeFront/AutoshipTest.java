...
public class AutoshipTest extends StoreFrontWebsiteBaseTest{
	...
	public void testUpdateAutoshipCartEditShipAddressPC_399(){
		...
		s_assert.assertFalse(sfCheckoutPage.getDefaultShippingAddressNameAtCheckoutPage().contains(lastName),
	 ...
	 *     
	 */
	@Test(enabled=false)//functionality changed for this test
	 ...
	 *     
	 */
	@Test(enabled=false)//No more a storefront test as per qtest
	...
	public void testVerifyUpdateChangesOnAutoShipCartAfterClickSave_509(){
		...
		sfCheckoutPage.clickShippingDetailsNextbutton();
		billingProfileNameFromFromUI = sfCheckoutPage.getDefaultBillingProfileName();
		s_assert.assertTrue(billingProfileNameFromFromUI.contains(cardName), "Expected billing profile name is "+cardName+" but actual on UI is "+billingProfileNameFromFromUI);
		s_assert.assertAll();
	}
	...
	public void testAddToAutoshipCartConsultant(){
		...
		sfShopSkinCarePage.checkoutTheCartFromPopUp();
		if(isProductPresent){
			updatedQuantity = sfAutoshipCartPage.updateQuantityByOne(productQuantity);
			updatedQuantityFromUI = sfAutoshipCartPage.getQtyOfProductFromAutoShipCart(validProductName);
			s_assert.assertTrue(updatedQuantityFromUI.equals(updatedQuantity), "Added item from PLP is not present into cart");
		}else{
			updatedQuantityFromUI = sfAutoshipCartPage.getQtyOfProductFromAutoShipCart(validProductName);
