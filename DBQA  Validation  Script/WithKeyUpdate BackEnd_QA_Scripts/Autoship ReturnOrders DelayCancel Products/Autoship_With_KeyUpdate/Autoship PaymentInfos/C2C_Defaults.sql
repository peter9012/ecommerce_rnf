...
SELECT  DISTINCT p.AutoshipPaymentID
INTO    #tempact
FROM    RFOperations.Hybris.AutoshipPayment p
JOIN Hybris..orders ho ON p.AutoshipID=ho.PK
AND ho.p_template=1 AND ho.currencypk=8796125855777
JOIN Hybris..paymentinfos hp ON hp.pk=p.AutoshipPaymentID
AND hp.p_sourcename='Hybris-DM'


CREATE CLUSTERED INDEX as_cls1 ON #tempact (AutoshipPaymentID);
...
SELECT  @StartedTime AS StartedTime ,
		...
		SELECT  'Autoship PaymentInfos Column to Column Validation Started ' AS EntityName ,
        GETDATE() AS StartedTime;
		SELECT  @StartedTime=CAST(GETDATE() AS TIME)
...
SELECT  'Autoship PaymentInfo Column to Columns Validation Completed' AS EntityName ,
        GETDATE() AS CompletionTime;
...
SELECT  @StartedTime AS StartedTime ,
        @EndTime AS CompletionTime ,
        DATEDIFF(MINUTE, @StartedTime, @EndTime) AS [Total Time (MM)] ,
        'Autoship PaymentInfos Column to Column Validation' AS Entity; 
...
INSERT  INTO DataMigration.dbo.ExecResult
		...
		SELECT  'Autoship PaymentInfos Default Column Validation Started ' AS EntityName ,
        GETDATE() AS StartedTime;
		SELECT  @StartedTime=CAST(GETDATE() AS TIME)
...
SELECT  'Autoship PaymentInfos default Columns Validation completed' AS EntityName ,
        GETDATE() AS CompletionTime;
...
SELECT  @StartedTime AS StartedTime ,
        @EndTime AS CompletionTime ,
        DATEDIFF(MINUTE, @StartedTime, @EndTime) AS [Total Time (MM)] ,
        'Autoship PaymentInfos Default Column Validation' AS Entity; 
