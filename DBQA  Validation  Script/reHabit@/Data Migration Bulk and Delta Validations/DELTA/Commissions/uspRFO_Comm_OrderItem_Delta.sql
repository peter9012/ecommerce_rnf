...
CREATE PROCEDURE [dbqa].[uspRFO_Comm_OrderItem_Delta] (@StartDate DATE,@EndDate DATETIME=NULL)
AS
    BEGIN
...
SET @EndDate=ISNULL(@EndDate,GETDATE()-1)
        ...
        DECLARE @SourceCount INT ,
            ...
            @owner NVARCHAR(50)= 'Comm_OrderItems' ,
            @Key NVARCHAR(25)= 'OrderID' ,
        ...
        SELECT  oi.OrderId [SourceKey] ,
                oi.OrderItemID AS r_OrderItemID ,
                p.SKU AS r_SKU ,
                oi.Quantity AS r_Quantity ,
                oi.CV AS r_CV ,
                oi.QV AS r_QV ,
                oi.WholesalePrice AS r_WholesalePrice ,
                oi.TotalPrice AS r_PricePaid ,
                oi.Quantity * oi.CV AS r_TotalCV ,
                oi.Quantity * oi.QV AS r_TotalQV ,
                oi.Quantity * oi.WholesalePrice AS r_TotalWholesalePrice ,
                oi.Quantity * oi.TotalPrice AS r_totalPricePaid ,
                oi.modifiedDate AS r_createdOn ,
                201702 AS r_PeriodID ,
                oi.modifiedDate AS r_UpdatedOn ,
                n.OrderNoteID AS r_CancellationReasonID
				INTO #Source
        FROM    RFOperations.Hybris.OrderItem oi
                JOIN RFOperations.Hybris.ProductBase p ON p.productID = oi.ProductID
                LEFT JOIN RFOperations.Hybris.OrderNotes n ON n.OrderID = oi.OrderId
				WHERE oi.ServerModifiedDate BETWEEN @StartDate AND @EndDate 
...
INTO    #Target
FROM    [Commissions].[etl].[OrderItems]
...
--++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      FROM    DM_QA.dbqa.Map_tab
        WHERE   [Owner] = 'rfo_comm_orderitems'
