...
SELECT  'Autoship Items Column to Column Validation Started ' AS EntityName ,
        GETDATE() AS StartedTime;
SELECT  @StartedTime = CAST(GETDATE() AS TIME);
...
SELECT b.' + @RFO_key + ', ' + RFO_Column + ' as RFO_Col FROM rfoperations.'
                + [Schema] + '.' + RFO_Table
                + ' a, #tempact b where a.autoshipid=b.autoshipid) A  

LEFT JOIN

(SELECT b.' + @RFO_key + ', ' + RFO_Column + ' as RFO_Col FROM rfoperations.'
                + [Schema] + '.' + RFO_Table
                + ' a, #tempact b where a.autoshipid=b.autoshipid
except
...
SELECT  'Autoship Items Columns to Columns Validation' AS EntityName ,
        GETDATE() AS CompletionTime;
...
SELECT  @StartedTime AS StartedTime ,
        @EndTime AS CompletionTime ,
        DATEDIFF(MINUTE, @StartedTime, @EndTime) AS [Total Time (MM)] ,
        'Autoship Items Columns to  Columns Validation' AS Entity; 
...
SELECT  'Autoship Items Default Columns Validation Started ' AS EntityName ,
        GETDATE() AS StartedTime;
SELECT  @StartedTime = CAST(GETDATE() AS TIME);
...
SELECT  'Autoship items default Columns Validation Completed' AS EntityName ,
        GETDATE() AS CompletionTime;
...
SELECT  @StartedTime AS StartedTime ,
        @EndTime AS CompletionTime ,
        DATEDIFF(MINUTE, @StartedTime, @EndTime) AS [Total Time (MM)] ,
        'Autoship Items Default Column Validation' AS Entity; 
