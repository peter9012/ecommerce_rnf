...
SELECT  ro.ReturnOrderID ,
        ro.AccountID ,
        u.PK ,
        CAST(rp.ReturnPaymentId AS NVARCHAR(255)) AS ReturnPaymentId ,
        rp.VendorID ,
...
FROM    RFOperations.Hybris.ReturnOrder ro
        ...
        JOIN Hybris.dbo.paymentinfos hpi ON hpi.code = CAST(rp.ReturnOrderID AS NVARCHAR)
...
GROUP BY ro.ReturnOrderID ,
        ro.AccountID ,
        u.PK ,
        CAST(rp.ReturnPaymentId AS NVARCHAR(255)) ,
        rp.VendorID ,
        ho.paymentinfopk ,
...
SELECT a.' + @RFO_key + ', ' + RFO_Column
                + ' as RFO_Trans_Col FROM  #tempact a) A  

LEFT JOIN

(SELECT a.' + @RFO_key + ', ' + RFO_Column
                + ' as RFO_Trans_Col FROM #tempact a
except
