...
public class MyAccountTest extends RFWebsiteBaseTest{
	...
	public void testVerifySpecialCharactersAreNotAllowed_4155() throws InterruptedException{
		...
		s_assert.assertTrue(storeFrontHomePage.verifySpecialCharNotAcceptInPrefixName(), "Special Char is accepcted by prefix name");
		...
		s_assert.assertTrue(storeFrontAccountInfoPage.verifySpecialCharNotAcceptInPrefixName(), "Special Char is accepcted by prefix name");
		storeFrontHomePage.enterWebsitePrefixName(prefixName);
		s_assert.assertFalse(storeFrontAccountInfoPage.verifySpecialCharNotAcceptInPrefixName(), "Special Char is accepcted by prefix name");
		s_assert.assertAll(); 
	}
	...
	public void testRegisterAsRCWithDifferentCASponserWithPulse_3774() throws InterruptedException {
		...
		storeFrontHomePage = new StoreFrontHomePage(driver);
		// Get Canadian sponser with PWS from database
		sponserList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguementPWS(DBQueries_RFO.GET_RANDOM_CONSULTANT_WITH_PWS_RFO,driver.getEnvironment(),driver.getCountry()),RFO_DB);
		sponserHavingPulse = String.valueOf(getValueFromQueryResult(sponserList, "AccountID"));
	...
	public void testCheckShippingAndHandlingFee_2144() throws SQLException, InterruptedException{
		...
		if(orderTotal<=999999){
			if(driver.getCountry().equalsIgnoreCase("CA")){
				...
				s_assert.assertTrue(deliveryCharges.equalsIgnoreCase("$15.00"),"Shipping charges on UI is not As per shipping method selected");
	...
	public void testPCAccountTermination_4803() throws InterruptedException{
		...
		storeFrontHomePage.clickOnBillingNextStepBtn();
		/*storeFrontHomePage.clickPlaceOrderBtn();
		  s_assert.assertTrue(storeFrontHomePage.verifyPCPerksTermsAndConditionsPopup(),"PC Perks terms and conditions popup not visible when checkboxes for t&c not 

	selected and place order button clicked");
		  logger.info("PC Perks terms and conditions popup is visible when checkboxes for t&c not selected and place order button clicked");*/
		storeFrontHomePage.clickOnPCPerksTermsAndConditionsCheckBoxes();
		storeFrontHomePage.clickPlaceOrderBtn();
		storeFrontHomePage.clickOnRodanAndFieldsLogo();
	...
	@Test
	public void testCancellationMessageforPCaccountTermination() throws InterruptedException{
		RFO_DB = driver.getDBNameRFO();
		List<Map<String, Object>> randomPCUserList =  null;
		String pcUserEmailID = null;
		String accountId = null;
	...
	public void testPlaceAdhocOrderFromSponsorPWSSiteCheckSponsorOfOrder_3765() throws InterruptedException{
		...
		List<Map<String, Object>> randomConsultantList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguementPWS(DBQueries_RFO.GET_RANDOM_CONSULTANT_WITH_PWS_RFO,driver.getEnvironment(),driver.getCountry()),RFO_DB);
	...
	public void testRCToPcAndPCToConsultantUnderSameAndDifferentSponsor_4718() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			// Get Canadian sponser with PWS from database
			List<Map<String, Object>> sponserList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguementPWS(DBQueries_RFO.GET_RANDOM_CONSULTANT_WITH_PWS_RFO,driver.getEnvironment(),driver.getCountry()),RFO_DB);
	...
	@Test
	public void testVerifyChangeSponsorFunctionalityForRC_5138() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			RFO_DB = driver.getDBNameRFO();
			int randomNum = CommonUtils.getRandomNum(10000, 1000000);
			String newBillingProfileName = TestConstants.NEW_BILLING_PROFILE_NAME+randomNum;
			...
			storeFrontHomePage.clickOnPlaceOrderButton();
			storeFrontHomePage.enterNewRCDetails(TestConstants.FIRST_NAME+randomNum, TestConstants.LAST_NAME+randomNum, password);
			storeFrontHomePage.enterMainAccountInfo(TestConstants.ADDRESS_LINE_1_CA, TestConstants.CITY_CA, TestConstants.PROVINCE_CA, TestConstants.POSTAL_CODE_CA, TestConstants.PHONE_NUMBER_CA);
			//get pws from query for assertion
			randomConsultantList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguement(DBQueries_RFO.GET_RANDOM_ACTIVE_CONSULTANT_WITH_ORDERS_AND_AUTOSHIPS_RFO,countryId),RFO_DB);
			String emailIdOfFirstConsultantSponser = (String) getValueFromQueryResult(randomConsultantList, "Username");  
			String accountIDOfFirstConsultantSponser = String.valueOf(getValueFromQueryResult(randomConsultantList, "AccountID"));
			// sponser search by Account Number
			sponsorIdList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguement(DBQueries_RFO.GET_ACCOUNT_NUMBER_FOR_PWS,accountIDOfFirstConsultantSponser),RFO_DB);
			String sponserId = (String) getValueFromQueryResult(sponsorIdList, "AccountNumber");
			storeFrontHomePage.enterSponsorNameAndClickOnSearchForPCAndRC(sponserId);
			storeFrontHomePage.mouseHoverSponsorDataAndClickContinueForPCAndRC();
			s_assert.assertTrue(storeFrontHomePage.verifyNotYourSponsorLinkPresent(),"NOT YOUR SPONSOR link is not present");
			storeFrontHomePage.clickOnNotYourSponsorLink();
			s_assert.assertTrue(storeFrontHomePage.verifyContinueWithoutSponsorLinkIsPresent(),"CONTINUE WITHOUT SPONSOR LINK is not present on UI");
			storeFrontHomePage.clickOnContinueWithoutSponsorLink();
			storeFrontHomePage.clickOnNextButtonAfterSelectingSponsor();
			storeFrontHomePage.clickOnShippingAddressNextStepBtn();
			storeFrontHomePage.enterNewBillingCardNumber(TestConstants.CARD_NUMBER);
	...
	@Test
	public void testAddBirthDay_Gender_AllowMySponsorInformation_5003() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			RFO_DB = driver.getDBNameRFO();
			storeFrontHomePage = new StoreFrontHomePage(driver);
			List<Map<String, Object>> accountNameDetailsList = null;
	...
	public void testInactiveRCEmailAddressDuringConsultantEnrollment_3963() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			s_assert.assertTrue(storeFrontHomePage.verifyUpradingToConsulTantPopupForRC(), "Upgrading to a consultant popup is not present");
			storeFrontHomePage.enterFirstName(firstName);
			storeFrontHomePage.enterLastName(lastName);
			storeFrontHomePage.enterEmailAddress(rcUserEmailID);
			storeFrontHomePage.enterPassword(password);
	...
	public void testActivePCEmailIdDuringConsultantEnrollmentUnderSameSponsor_3958() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			while(true){
				randomPCUserList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguement(DBQueries_RFO.GET_RANDOM_ACTIVE_PC_WITH_ORDERS_AND_AUTOSHIPS_RFO,countryId),RFO_DB);
				pcEmailId = (String) getValueFromQueryResult(randomPCUserList, "UserName");  
				accountId = String.valueOf(getValueFromQueryResult(randomPCUserList, "AccountID"));
				logger.info("Account Id of the user is "+accountId);
