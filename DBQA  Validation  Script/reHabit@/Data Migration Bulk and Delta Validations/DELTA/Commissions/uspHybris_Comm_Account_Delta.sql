...
ALTER PROCEDURE [dbqa].[uspHybris_Comm_Account_Delta] (@StartDate DATE,@EndDate DATETIME=NULL)
AS
    BEGIN
...
SET @EndDate=ISNULL(@EndDate,GETDATE()-1)
        ...
        SELECT  u.p_customerid [SourceKey],
                su.p_customerid p_sponsorid ,
                su.p_customerid P_enrollerId ,
                u.p_enrollmentdate ,
                s.Code p_accountstatus ,
                t.Code p_AccountType ,
                cu.p_isocode p_currency ,
        ...
        FROM    hybris.dbo.users u
                LEFT JOIN Hybris.dbo.users su ON su.pk = u.p_newsponsorid
        ...
        WHERE   t.Code IN ( 'retail', 'preferred', 'consultant' )
...
SELECT  CAST([AccountID] AS NVARCHAR(225)) [TargetKey],
        [SponsorID] ,
        [EnrollerID] ,
        [EnrollmentDate] ,
        [StatusID] ,
        ...
        [Active] ,
        [AccountNumber] ,
        [PlacementSponsorAccountNumber] ,
        [EnrollmentSponsorAccountNumber]
INTO    #Target
FROM    [Commissions].[etl].[Accounts]
