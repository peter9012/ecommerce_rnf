		...
		SELECT  *
		FROM    #missing m;
				--JOIN hybris..orders o ON m.pk=o.pk
...
SELECT  'Validation of column to column with HybrisOrder in progress' AS [Step-2 Validation] ,
				GETDATE() AS StartTime;
				SET  @HYB_key  = 'code';
		...
		--Defaults Check
		SELECT  'Step-2 Completed, Validation of default columns for Hybris_Order in progress' AS [Step-3 Validation] ,
				GETDATE() AS StartTime;
