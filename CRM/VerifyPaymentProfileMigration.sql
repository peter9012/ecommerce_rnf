...
FROM 
    (SELECT PaymentProfileId FROM Rfoperations.rfo_accounts.AccountBase AB, RFOPERATIONS.RFO_ACCOUNTS.PaymentProfiles pp WHERE PP.ACCOUNTID=AB.ACCOUNTID) a
    FULL OUTER JOIN 
    (SELECT RFOPaymentProfileId__C FROM sfdcbackup.SFDCBKP.PaymentProfile PP,SFDCBACKUP.SFDCBKP.Accounts A WHERE PP.ACCOUNT__C=A.ID AND CAST(PP.LASTMODIFIEDDATE AS DATE) >= @LastRunDate) b 
...
SELECT
		...
		CCV.NAME AS Vendor,
		CAST(CASE WHEN PP.StartDate IS NULL THEN '1900-01-01' ELSE pp.StartDate END AS DATE) StartDate,
		CAST(CASE WHEN PP.EndDate IS NULL THEN '1900-01-01' ELSE pp.EndDate END AS DATE) EndDate,
		...
		CAST(CAST(CASE WHEN LEN(AA.latitude)<1 THEN NULL ELSE AA.latitude END AS DECIMAL(1,1)) AS NVARCHAR(MAX)) latitude,
		CAST(CASE WHEN LEN(AA.locale) <1 THEN NULL ELSE AA.locale END AS NVARCHAR(MAX)) locale,
		CAST(CAST(CASE WHEN LEN(AA.longitude)<1 THEN NULL ELSE AA.longitude END AS DECIMAL(1,1)) AS NVARCHAR(MAX)) longitude,
...
FROM sfdcbackup.SFDCBKP.PaymentProfile PP, SFDCBACKUP.SFDCBKP.COUNTRY C ,
	 SFDCBACKUP.SFDCBKP.Accounts A WHERE PP.ACCOUNT__C=A.ID AND C.ID=PP.Country__c
...
DECLARE @I INT = (SELECT MIN(ColID) FROM  Rfoperations.sfdc.CRM_Metadata_Accounts WHERE CRMObject = 'PaymentProfile') , 
@C INT =  (SELECT MAX(ColID) FROM  Rfoperations.sfdc.CRM_Metadata_Accounts WHERE CRMObject = 'PaymentProfile') 
...
FROM rfoperations.sfdc.ErrorLog_PaymentProfiles A JOIN Rfoperations.sfdc.CRM_Metadata_Accounts B ON a.ColID =b.ColID
